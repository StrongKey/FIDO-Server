...
public class WebAuthnModelAuthenticator {
    ...
    public PublicKeyCredentialSource lookup(byte[] credentialId) {
        ...
        if (!isCapableOfStoringClientSideResidentCredential()) {
            try {
                ...
                credentialSource = jsonMapper.readValue(credentialId,PublicKeyCredentialSource.class);
                credentialSource.setId(credentialId);
                return credentialSource;
            } catch (IOException ex) {
                throw new WebAuthnModelException(ex);
            }
        }
    ...
    public MakeCredentialResponse makeCredential(MakeCredentialRequest makeCredentialRequest, RegistrationEmulationOption registrationEmulationOption) {
        ...
        try {
// DBP            KeyPair keyPair = KeyUtil.createECKeyPair();

            byte[] seed = makeCredentialRequest.getUserEntity().getId();
            KeyPair keyPair = KeyUtil.createECKeyPair(seed);
            credentialPrivateKey = keyPair.getPrivate();
            System.out.println("PRIVATE KEY: "+Base64UrlUtil.encodeToString(credentialPrivateKey.getEncoded()));
            ...
            // Otherwise:
                ...
                jsonMapper = ObjectMapperUtil.createJSONMapper();
                try {
                    credentialId = jsonMapper.writeValueAsBytes(credentialSource);
        ...
        AuthenticatorData authenticatorDataObject = new AuthenticatorData(rpIdHash, flag, counter, attestedCredentialData, processedExtensions);
...
//        AttestationStatement attestationStatement = new NoneAttestationStatement();
        ...
        AttestationObject attestationObject = new AttestationObject(authenticatorDataObject, attestationStatement);
