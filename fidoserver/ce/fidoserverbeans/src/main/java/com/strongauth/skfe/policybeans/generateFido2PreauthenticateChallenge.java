...
import com.strongauth.skfe.fido.policyobjects.extensions.Fido2AuthenticationExtension;
...
public class generateFido2PreauthenticateChallenge implements generateFido2PreauthenticateChallengeLocal {
    ...
    public String execute(Long did, String username, JsonObject options, JsonObject extensions) {
        ...
        try {
            ...
            returnObjectBuilder.add(skfeConstants.FIDO2_PREAUTH_ATTR_CHALLENGE, challenge)
                    .add(skfeConstants.FIDO2_PREAUTH_ATTR_ALLOWCREDENTIALS,
                            generateAllowCredentialsList(fidoPolicy.getAuthenticationOptions(), fks));
        
            //Add optionals
            if (fidoPolicy.getTimeout() != null) {
                returnObjectBuilder.add(skfeConstants.FIDO2_PREAUTH_ATTR_TIMEOUT, fidoPolicy.getTimeout());
            }
            String rpId = generateRpId(fidoPolicy.getRpOptions());
            if (rpId != null) {
                returnObjectBuilder.add(skfeConstants.FIDO2_PREAUTH_ATTR_RPID, rpId);
            }
            ...
            if (userVerificationPref != null) {
                returnObjectBuilder.add(skfeConstants.FIDO2_PREAUTH_ATTR_UV, userVerificationPref);
            }
            ...
            for(FidoKeys fk : fks){
                String KHHash = skfeCommon.getDigest(fk.getKeyhandle(), "SHA-256");
                UserSessionInfo session = new UserSessionInfo(username,
                        challenge, fk.getAppid(), skfeConstants.FIDO_USERSESSION_AUTH, fk.getPublickey(), "");
    ...
    private String getUserId(Long did, String username) throws SKFEException {
        ...
        if (fk == null) {
            ...
            throw new SKFEException(skfeCommon.getMessageProperty("FIDO-ERR-0007"));
        } else {
            return fk.getUserid();
        }
    }
    ...
    private JsonArray generateAllowCredentialsList(AuthenticationPolicyOptions authOp, Collection<FidoKeys> fks) throws SKFEException {
        ...
        for (FidoKeys fk : fks) {
            if (authOp.getAllowCredentials() != null && 
                    authOp.getAllowCredentials().equalsIgnoreCase(skfeConstants.POLICY_CONST_ENABLED)) {
...
//                    }
    ...
    private String generateUserVerification(AuthenticationPolicyOptions authOp, JsonObject options){
        ...
        String rpRequestedUserVerification = options.getString(skfeConstants.FIDO2_ATTR_USERVERIFICATION, null);
        ...
        if (userVerificationResponse == null && !authOp.getUserVerification().contains(skfeConstants.POLICY_CONST_PREFERRED)) {
