 ...
 *************************************************
 *
 * An encryption key-management infrastructure (EKMI) must have a minimum of
 * two servers, but may have an unlimited number of servers.  Each server
 * must have a unique FQDN and numeric server identifier.  Additionally, the
 * pseudo-numbers returned by each server must be aunique range that is NOT
 * returned by any other server within the EKMI.
 */

package com.strongauth.appliance.entitybeans;
