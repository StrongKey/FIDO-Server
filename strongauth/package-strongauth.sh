...
# $URL$
#
# Script to create the SKFE Bundle
#
###############################################################
...
replresources=$strongauth/Common/src/main/resources/resources/repl
replmessages=$(sed -n '31,$p' $replresources/replication-messages.properties)

skccresources=$strongauth/Common/src/main/resources/resources/skcc
skccmessages=$(sed -n '20,$p' $skccresources/skccmessages.properties)
...
fsoresources=$strongauth/Common/src/main/resources/resources/fso
fsomessages=$(sed -n '31,$p' $fsoresources/fso-messages.properties)
saftmessages=$(sed -n '31,$p' $fsoresources/saft-messages.properties)
scmmessages=$(sed -n '31,$p' $fsoresources/scm-messages.properties)
...
failure() {
        ...
        echo "There was a problem creating the SKFE distribution. Aborting." >&2
        tty -s && tput sgr0
        exit 1
}
...
cp $strongauth/strongauthEAR/target/skfe.ear $strongauth/jade

# Create archives
echo "-Packaging jade..."
tar zcf SKFE-${version}.tgz -C $strongauth jade
