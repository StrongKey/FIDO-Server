...
public class CryptographyPolicyOptions {
    ...
    public static CryptographyPolicyOptions parse(JsonObject cryptoJson) {
        CryptographyPolicyOptionsBuilder cryptoPolicyBuilder = 
                ...
                new ArrayList<>(cryptoJson.getJsonArray(skfeConstants.POLICY_CRYPTO_ATTESTATION_FORMATS).stream().map(x -> ((JsonString) x).getString()).collect(Collectors.toList())),
                new ArrayList<>(cryptoJson.getJsonArray(skfeConstants.POLICY_CRYPTO_ATTESTATION_TYPES).stream().map(x -> ((JsonString) x).getString()).collect(Collectors.toList())));
        if(cryptoJson.getJsonArray(skfeConstants.POLICY_CRYPTO_ALLOWED_EC_SIGNATURES) != null){
            cryptoPolicyBuilder.setAllowedECSignatures(new ArrayList<>(cryptoJson.getJsonArray(skfeConstants.POLICY_CRYPTO_ALLOWED_EC_SIGNATURES).stream().map(x -> ((JsonString) x).getString()).collect(Collectors.toList())));
        }
        if(cryptoJson.getJsonArray(skfeConstants.POLICY_CRYPTO_ALLOWED_RSA_SIGNATURES) != null){
            cryptoPolicyBuilder.setAllowedRSASignatures(new ArrayList<>(cryptoJson.getJsonArray(skfeConstants.POLICY_CRYPTO_ALLOWED_RSA_SIGNATURES).stream().map(x -> ((JsonString) x).getString()).collect(Collectors.toList())));
        }
        if(cryptoJson.getJsonArray(skfeConstants.POLICY_CRYPTO_ELLIPTIC_CURVES) != null){
            cryptoPolicyBuilder.setSupportedEllipticCurves(new ArrayList<>(cryptoJson.getJsonArray(skfeConstants.POLICY_CRYPTO_ELLIPTIC_CURVES).stream().map(x -> ((JsonString) x).getString()).collect(Collectors.toList())));
        }
