...
-  **The scripted installation process has been tested on Ubuntu 18.04 LTS only.** The installation script is untested on other flavors of Linux but may work with slight modifications.
...
0a.  **Set locale for the machine to en_US. Run the following commands:
    
        ```sh
            sudo update-locale LANG=en_US.UTF-8 LANGUAGE=en_US
        ```

0b.  **Restart the machine for the new language and locale to take effect.
...
2.  **Download and install** various dependencies. Run the following commands:
    ...
    ```sh
    sudo apt-get update
    sudo apt install wget unzip libaio1 openjdk-8-jdk daemon rng-tools
    ```
