...
MYSQL_SKLES_PASSWORD=$(grep \"password\" /usr/local/strongauth/glassfish4/glassfish/domains/domain1/config/domain.xml | sed "s|.*value=\"\(.*\)\".*|\1|" | head -1)
SERVICE_LDAP_BIND_PASS=$(sed -rn 's|^ldape.cfg.property.service.cc.ldap.ldapbinddn.password=(.*)|\1|p' /usr/local/strongauth/skcc/etc/skcc-configuration.properties)
SERVICE_LDAP_BASEDN=$(sed -rn 's|^ldape.cfg.property.service.cc.ldap.basedn=(.*)|\1|p' /usr/local/strongauth/skcc/etc/skcc-configuration.properties)
SAKA_DID=$1
