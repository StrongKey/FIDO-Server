...
//    }
  ...
  wrapOpenClose: function(a, cur_list_el) {
    ...
    return function(e) {
      ...
      // added this line to style ol tags in TOC
      
      ol.classname = (d == 'collapsible');
      //end added content
...
(function(i) {
	...
	document.addEventListener("DOMContentLoaded",i,false);
		...
		(
			function(){var t=document.createElement('doc:rdy');
			try{t.doScroll('left');
			i();
			t=null;}catch(e){st(arguments.callee,0);}})();
	} else{
		window.onload=i;
	}
})
...
/* numbering counters for the <ol> tags in the TOC */
/* should be identical to the <h#> tag header numbers generated in CSS */
...
/* added a title builder based on meta tag values */
function SKDocTitle() {
    var version_major = $('meta[name=Version_Major]').attr("content");
    var version_minor =	$('meta[name=Version_Minor]').attr("content");
    var version_patch =	$('meta[name=Version_Patch]').attr("content");
    var version_build =	$('meta[name=Version_Build]').attr("content");
    var product_group =	$('meta[name=Product_Group]').attr("content");
    var product =	$('meta[name=Product]').attr("content");
    var abbr =	$('meta[name=Abbr]').attr("content");
    var organization =	$('meta[name=Organization]').attr("content");
    var classification =$('meta[name=classification]').attr("content");
    var scripted =	$('meta[name=Scripted]').attr("content");
    
    if(abbr==""){
    	var doctitle = product + "<br> " + classification;
        var toctitle = product + "<br>" + classification;
    }
    else if(abbr!=""){
    	var doctitle = product + " (" + abbr + ")<br> " + classification;
        var toctitle = product + " (" + abbr + ")<br>" + classification;
    }
   
    var docversion = doctitle + " " + version_major + "." + version_minor;
 
    if(scripted!=""){
    	var docversion = docversion + scripted;
    	var doctitle = doctitle + scripted;
        var toctitle = toctitle + scripted;
    }
    
   
    document.getElementById("doctitle_print").innerHTML = doctitle;
    document.getElementById("doctitle_screen").innerHTML = toctitle;
    document.getElementById("closing_title").innerHTML = docversion;
    document.getElementById("footer_title").innerHTML = doctitle;
}
/* end title builder */
